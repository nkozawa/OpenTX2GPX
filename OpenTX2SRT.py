...
def main():
    ...
    for i in range(10):
        ordrow.append([sg.Text('',key='otItem'+str(i)),sg.Button('^',key='bUp'+str(i),enable_events=True),sg.Button('v',key='bDown'+str(i),enable_events=True)])
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            break
        elif event == 'tPath':                      # line1 filename processing
            print(">"+values['tPath']+"<")
            validLogFile = False
            ...
            if (len(values['tPath']) > 0):
                ...
                if (otext[1].lower() == '.csv'):
                    ...
                    validLogFile = True
        elif event == 'bProc':                      # line2 open log file
            if (validLogFile):
